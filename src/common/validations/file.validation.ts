...
export class FileValidationPipe implements PipeTransform {
  ...
  constructor(options?: FileValidationOptions) {
    ...
    this.allowedTypes = options?.allowedTypes ?? ["image/png", "image/jpg", "image/jpeg"];
  ...
  transform(file: Express.Multer.File | Express.Multer.File[], metadata: ArgumentMetadata) {
    ...
    if (this.isArray) {
      ...
      if (files.length === 0) {
        throw new BadRequestException("لیست فایل‌ها خالی است");
      }
  ...
  private validateFile(file: Express.Multer.File): Express.Multer.File {
    if (file.size > this.maxSize) {
      throw new BadRequestException(
        `حجم فایل نباید بیشتر از ${(this.maxSize / (1024 * 1024)).toFixed(1)} مگابایت باشد`,
      );
    }

    if (!this.allowedTypes.includes(file.mimetype)) {
      throw new BadRequestException(
        `فرمت فایل معتبر نیست. فقط ${this.allowedTypes.join(", ")} مجاز هستند`,
      );
    }
